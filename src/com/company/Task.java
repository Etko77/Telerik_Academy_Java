...
public class Task extends BoardItem {
    ...
    public void revertStatus() {
        if (this.getStatus() != Status.TODO){//First Enum Value to be used
            setStatus(Status.values()[this.getStatus().ordinal() - 1]);
        }else{
    ...
    public void advanceStatus() {
        if (this.getStatus() != Status.VERIFIED){//Last Enum Value
            setStatus(Status.values()[this.getStatus().ordinal() + 1]);
        }else{
