...
public class RemoveCommentCommand extends BaseCommand {
    ...
    protected String executeCommand(List<String> parameters) {
        ...
        String vehicleOwnerUsername = parameters.get(2);

        return removeComment(vehicleIndex, commentIndex, vehicleOwnerUsername);
    }

    private String removeComment(int vehicleIndex, int commentIndex, String vehicleOwnerUsername) {
        ...
        ValidationHelpers.validateIntRange(vehicleIndex, 0, vehicleOwner.getVehicles().size() - 1, VEHICLE_INDEX_OUT_OF_BOUNDS);
        ValidationHelpers.validateIntRange(commentIndex, 0, vehicleOwner.getVehicles().get(vehicleIndex).getComments().size() - 1, COMMENT_INDEX_OUT_OF_BOUNDS);

        Vehicle vehicle = vehicleOwner.getVehicles().get(vehicleIndex);
        Comment comment = vehicleOwner.getVehicles().get(vehicleIndex).getComments().get(commentIndex);
